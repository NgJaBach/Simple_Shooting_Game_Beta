...
#define music(a) PlaySound(a,NULL,SND_ASYNC|SND_FILENAME|SND_LOOP);
using namespace std;
...
bool ok=false,progress[10],ene_coord=false,theme=true;
...
void not_availble(){
	color(143);
	gotoxy(12,19);printf("                                                                           ");
	gotoxy(12,20);printf(" Your key is Invalid or this game_mode/funtion is still not completed yet! ");
	gotoxy(12,21);printf("                            Try other key word!                            ");
	gotoxy(12,22);printf("                                                                           ");
	getch();
}
void retry(){
...
void addmap(int lev){string s;
	if(lev==1){
		...
		if(theme) music("beginning.wav");
	}
	else if(lev==2){
		if(theme) music("easy.wav");
		m=70;
		myfile.open("level2.txt");
	}
	else if(lev==3){
		if(theme) music("norm.wav");
		m=60;
		myfile.open("level3.txt");
	}
	else if(lev==4){
		if(theme) music("hard.wav");
		m=70;
		myfile.open("level4.txt");
	}
	else if(lev==5){
		if(theme) music("lunatic.wav");
		m=59;
...
void sound(){
	...
	gotoxy(25,8);printf("                                                        ");
	gotoxy(25,9);printf("                                                        ");
		gotoxy(27,8); printf("Game sound is currently "); color(112);printf("|");
		if(!theme){color(120);printf("OFF"); }
		else{color(118);printf("ON");}
		color(112);printf("|");
		gotoxy(49,11); printf("Yes");
		gotoxy(49,12);printf("NO ");
...
void menu(){if(theme) music("menu.wav");
	for(;;){
			...
			case ':':{
				...
				for(;;){
					if(choice==13){
						...
						color(112);system("cls");es();color(143);gotoxy(40,5);printf("Battle Settings");
						gotoxy(33,7); printf("Sound Settings");
	...
	for(int i=0;i<number;i++){
		if(pos[ene[i].First][ene[i].Second]==i) if(a[ene[i].First][ene[i].Second]=='E'){
			...
			if(x<ene[i].First&&a[ene[i].First-1][ene[i].Second]!='E'){
				...
				if(a[ene[i].First-1][ene[i].Second]!='E'){
					if(a[ene[i].First-1][ene[i].Second]=='P') approach2(ene[i].First,ene[i].Second);
					swap(a[ene[i].First-1][ene[i].Second],a[ene[i].First][ene[i].Second]);
					gotoxy(ene[i].Second,ene[i].First); printf(" ");
					gotoxy(ene[i].Second,ene[i].First-1); printf("E");
					swap(pos[ene[i].First-1][ene[i].Second],pos[ene[i].First][ene[i].Second]);
					--ene[i].First;}
				else if(a[ene[i].First+1][ene[i].Second]!='E'){
					if(a[ene[i].First+1][ene[i].Second]=='P') approach2(ene[i].First,ene[i].Second);
					swap(a[ene[i].First+1][ene[i].Second],a[ene[i].First][ene[i].Second]);
					gotoxy(ene[i].Second,ene[i].First); printf(" ");
					gotoxy(ene[i].Second,ene[i].First+1); printf("E");
					swap(pos[ene[i].First+1][ene[i].Second],pos[ene[i].First][ene[i].Second]);
					++ene[i].First;}
				else if(a[ene[i].First][ene[i].Second+1]!='E'){
					if(a[ene[i].First][ene[i].Second+1]=='P') approach2(ene[i].First,ene[i].Second);
					swap(a[ene[i].First][ene[i].Second+1],a[ene[i].First][ene[i].Second]);
					gotoxy(ene[i].Second,ene[i].First); printf(" ");
					gotoxy(ene[i].Second+1,ene[i].First); printf("E");
...
void mune(){if(theme) music("file.wav");color(0);system("cls"); gotoxy(0,0); 
	color(144);printf(
	"	              \n"
	"	   Esc-Quit   \n"
	"	              \n"
 ...
 color(384);
	...
	myfile.close();if(theme) music("menu.wav");
	/*
	File's name
